...
export function usePerformanceTableData({
  ...
  const loadPerformanceData = useCallback(async () => {
    ...
    console.log('[PERF-TABLE-FIXED] loadPerformanceData called with auto-fix enabled');
    ...
    if ((!reportId && !useConsolidatedView) || groupByDimensions.length === 0) {
      console.log('[PERF-TABLE-FIXED] No data loading - missing reportId/reportIds or groupByDimensions');
      setTableData([]);
      setTotalData({});
      setTotalCompareData({});
      setTotalChangeData({});
    ...
    try {
      ...
      console.log('[PERF-TABLE-FIXED] Applied auto-fix to', fixedRows.length, 'rows');

      // Apply vlookup mappings AFTER dimension fix
      if (vlookupMappings.length > 0) {
        for (const row of fixedRows as any[]) {
      ...
      // Calculate totals from transformed rows
      ...
      console.log('[PERF-TABLE-FIXED] Successfully loaded', transformedRows.length, 'rows with auto-fix');

    } catch (error) {
      console.error('[PERF-TABLE-FIXED] Error in loadPerformanceData:', error);
