...
- **Breakdown Analysis table** (`UnifiedBreakdownTable` in `BreakdownTableSection.tsx`): includes a **Gross Profit** column (per group row, expanded drill-down, and total). With Performance Model active, per-row commission split matches KPI math (`isPerformanceModelView` from `SlideViewPage` → `ChannelTab`); otherwise gross profit uses `calculateDerivedMetrics` (15% revenue − cost).
...
- **Default date filter:** Shared reports (both Data Studio and classic) default to "This Month" on initial load
